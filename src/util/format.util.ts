...
import wrap, { IOptions } from "../../node_modules/word-wrap/index";
...
export function maxSubjectLength(
  answers: Answers,
...
export function formatSubject(
  subject: string,
  disableSubjectToLowerCase: boolean
): string {
  let formatted = subject.trim();
  if (!disableSubjectToLowerCase) {
    formatted =
      formatted.charAt(0).toLowerCase() + formatted.slice(1, formatted.length);
  }
...
export function formatCommitMessage(
  ...
  const body = wrap(
    (answers.body || answers.breakingBody || answers.issueBody)!,
    wrapOptions
  );
  const breaking = !answers.isBreaking
    ? false
    : wrap(`BREAKING CHANGE: ${answers.breaking?.trim()}`, wrapOptions);
  const issues = answers.isIssueAffected
    ? wrap(answers.issueReference!.trim(), wrapOptions)
    : false;
